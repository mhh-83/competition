...
func _ready():
	if dir == 1:
		$TextureButton.position = Vector2(- $TextureButton.size.x * 2, 10)
	else:
		$TextureButton.position = Vector2(($TextureButton.size.x * 4) + size.x, 10)
...
func _on_texture_button_pressed():
	...
	if sellect_num == correct_sellect:
		...
		add_theme_stylebox_override("normal", preload("res://styles/test_btn_t.tres"))
		target.add_theme_stylebox_override("normal", preload("res://styles/test_btn_t.tres"))
		
	else:
		var noise = NoiseTexture2D.new()
		noise.noise = $Line2D.texture.noise
		noise.color_ramp = preload("res://styles/match_option_f.tres")
		add_theme_stylebox_override("normal", preload("res://styles/test_btn_f.tres"))
		target.add_theme_stylebox_override("normal", preload("res://styles/test_btn_f.tres"))
		$Line2D.texture = noise
